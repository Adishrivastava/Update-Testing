...
// ? ____________ authentication for admin ____________
...
$('#myModal222').modal({ backdrop: 'static', keyboard: false });
...
db.collection('dash1')
   ...
   .then(function (doc) {
      ...
      gendisplay.push({
         name: 'Stone Staged',
         value2: general.Stone.staged.toString(),
      });
      gendisplay.push({
         name: 'Drivers Onduty',
         value2: Object.keys(general.vehicledriver.onduty).toString(),
      });
      gendisplay.push({
         name: 'Vehicles Onduty',
         value2: Object.keys(general.vehiclesonduty).toString(),
      });
      gendisplay.push({
         name:
            'Total Hazira of=' +
            general.hazira.totalworkers +
            ' workers',
         value2: general.hazira.totalwedge,
      });
...
db.collection('Settings')
   ...
   .then(function (doc) {
      ...
      db.collection('Accounts/mdb/main')
         .where('date', '>=', yts)
         .get()
         .then(function (querySnapshot) {
            querySnapshot.forEach(function (doc) {
               ...
               if (type == 'Outflow') {
                  ...
                  if (ofcd[noc] == undefined) ofcd[noc] = {};
                  ...
                  if (ofud[user3] == undefined) ofud[user3] = {};
...
<div class="card-body"><h3>Balance:${
                  balances.balancelist[shorten(element) + 'bal']
                  }</h3><hr><h5>Inflow:${
                  ifc[element] || 'N.A'
                  }<hr>Outflow:${ofc[element] || 'N.A'}<hr>
...
<div class="card-header" ><h5>${element}</h5></div>
<div class="card-body"><h3>Balance:${
                  balances.balancelist[element + 'bal']
                  }</h3><hr><h5>Inflow:${
                  ifu[element] || 'N.A'
                  }<hr>Outflow:${ofu[element] || 'N.A'}<hr>
...
<div class="card-body"><h6>${doc.data().name}<hr><strong>Advance:${
               doc.data().advance
               }</strong><hr>${t2}<hr>${doc.data().placeofsupply}</h6> 
<button type="button" class="btn btn-success" id="${
               doc.id
               }" onclick="getid(this.id)" data-toggle="modal" data-target="#preordermod">Deliver</button>
<button type="button" class="btn btn-primary"  onclick="getid2(this.id)" class="ordercancel" id="${
               doc.id
               }">Cancel</button>    
    </div>
</div>`;
...
<button type="button" class="btn btn-success"id="${
               doc.id
               }" onclick="getid4(this.id)" data-toggle="modal" data-target="#postordermod">Deliver</button>
<button type="button" class="btn btn-primary"  onclick="getid2(this.id)" class="ordercancel" id="${
               doc.id
               }">Cancel</button>        
    </div>
  ...
  <div class="card-body">
    ...
    <button class="btn btn-primary" onclick="getid(this.id)" id=${
                  doc.id
                  } data-toggle="modal" data-target="#updatetodo">Update</button>
  </div>
</div>`;
            });
            ...
            db.collection(
               'dash1/vehicledetails/selfoutflow'
            ).onSnapshot(function (querySnapshot) {
               selfveh = [];
               querySnapshot.forEach(function (doc) {
                  console.log(general.vehiclesonduty);
                  if (
                     Object.keys(general.vehiclesonduty).includes(
                        doc.data().regno
                     )
                  )
            ...
            db.collection('dash1/vehicledetails/stoneinflowtrips')
               .where('time', '>=', currentts)
               .onSnapshot(function (querySnapshot) {
                  stoneveh = [];
                  querySnapshot.forEach(function (doc) {
            ...
            db.collection('notes')
               .where('date', '<=', todaysdate(4))
               .onSnapshot(function (querySnapshot) {
                  otherarr = [];
                  querySnapshot.forEach(function (doc) {
                     otherarr.push(new Date(parseInt(doc.id)));
                  });
                  console.log(otherarr);
                  replacelist('notesdate', otherarr);
               });
...
$(document).ready(function () {
   ...
   $('#btnaccess').click(function () {
      ...
      if (sum == balances.keywrd) {
         ...
         const x = sessionStorage.getItem('uid2');
   ...
   $('#userlogout').click(function () {
      ...
      } else swal('Error', 'no user Signed In', 'error');
   });

   $('#stone2').show();

   var selectas, astyp, temp;
   temp5 = 'none';
   ...
   $('#form5').submit(function (e) {
      ...
      if (parseInt($('#remfrequency option:selected').val()) == 1) {
         details55 = {
            ...
            remfreq: parseInt(
               $('#remfrequency option:selected').val()
            ),
            title: $('#tasktitle').val(),
            description: $('#taskdescription').val(),
            priority: parseInt(
               $('#rempriority option:selected').val()
            ),
            user: user,
         };
      } else {
   ...
   $('#form65').submit(function (e) {
      ...
      db.collection('Compandtodo')
         ...
         .then(function (doc) {
            ...
            if (
               ($('#taskupdatetype option:selected').val() == 1 &&
                  obj33.remfreq == 1) ||
               $('#taskupdatetype option:selected').val() == 4
            ) {
               db.collection('Compandtodo')
                  .doc(idi1)
                  .delete()
               ...
               console.log(
                  parseInt(obj33.date) + '+' + parseInt(obj33.interval)
               );
               obj33.date =
                  parseInt(obj33.date) + parseInt(obj33.interval);
               console.log(
               ...
               obj33.lasttaskdt =
                  parseInt(obj33.lasttaskdt) +
                  parseInt(obj33.interval);
   ...
   $('#form8').submit(function (e) {
      ...
      if (predel == 2) {
         ...
         if (
            !general.vehiclesonduty.hasOwnProperty(
               $('#prevehassigned').val()
            )
         )
            general.vehiclesonduty[$('#prevehassigned').val()] = 0;
         if (unload > 0) odd.advance = odd.advance + unload;
      }
               ...
               .then(function () {
                  ...
                  if (invtyp == 1)
                     window.location.href =
                        'invoicegen-half -billofsupply.html';
                  else if (invtyp == 2)
   ...
   $('#form89').submit(function (e) {
      ...
      rembal = odd.balance - odd.advance - postpay;
      ...
      if (postpay > 0) {
         ...
         if (
            odd.predelivery.invtyp == '2' ||
            odd.predelivery.invtyp == '4'
         ) {
            //cash payment
            ...
            if ($('#postpaytype option:selected').val() == '1') {
               ...
               details5 = {
                  ...
                  noc: odd.noc,
                  perticulars:
                     'Payment recieved against Inv:' +
                     odd.postdelivery.invno,
                  vouchartyp: 'Sales',
                  bankdebit: '',
                  cashdebit: '',
                  bankcredit: '',
            ...
            //E-transfer or Cheque
               ...
               $('#postpaytype option:selected').val() == '2' ||
               $('#postpaytype option:selected').val() == '3'
            ) {
               balances.balancelist['f' + noc4 + 'bal'] += postpay;
               details5 = {
                  tid: todaysdate(2),
               ...
               $('#postpaytype option:selected').val() == '4'
            ) {
               //unpaid
               balances.balancelist['f' + noc4 + 'bal'] += postpay;
               details5 = {
      ...
      if ($('#postpaytype option:selected').val() == '1') {
         ...
         details2 = {
            ...
            date:
               Date.parse($('#postremdate').val()) - 172800000 ||
               premdate.getTime() - 52415250,
            remfreq: 1,
         ...
         db.collection('debt')
            ...
            .then(function (doc) {
               if (doc.exists) {
                  ...
                  var tmp99;
                  if (
                     odd.typeofcustomer == 'Institutional-New' ||
                     odd.typeofcustomer == 'Institutional-Old'
                  )
                     tmp99 = 'Institutional';
                  else tmp99 = odd.typeofcustomer;
               ...
               var Ref224 = db
                  .collection('debt')
                  .doc(shorten2(odd.noc) + odd.contactdetails);
      ...
      //add reminder //for etransfer and cheque recordvbank details
      else {
      ...
      if (odd.predelivery.vehregno != undefined) {
         ...
         if (odd.predelivery.unloadcharges > 0) {
            if (exp > 0 || hazirabal > 0)
               notes +=
                  ...
                  'for order of' +
                  odd.name +
                  ' extrapayment in hazira=' +
                  hazirabal +
                  ' in misc exp:' +
                  exp;

            odd.predelivery.unloadcharges =
               parseFloat(odd.predelivery.unloadcharges) + hazirabal;
            balances.balancelist[odd.noc + 'bal'] -=
               odd.predelivery.unloadcharges;
            balances.balancelist['masterbalance'] -=
               odd.predelivery.unloadcharges;
            balances.balancelist[user + 'bal'] -=
         ...
         general.vehiclesonduty[odd.predelivery.vehregno] =
            general.vehiclesonduty[odd.predelivery.vehregno] + dis;
      ...
      if (odd.tradetype == 'Sale') {
         Object.keys(k2).forEach(function (element23) {
            balances.Products[odd.noc][element23]['qty'] =
               balances.Products[odd.noc][element23]['qty'] -
               k2[element23]['qty'];
            balances.Products[odd.noc][element23]['value'] =
               balances.Products[odd.noc][element23]['value'] -
               k2[element23]['value'];
            ...
            stockinfo['prostock'] =
               balances.Procurement[odd.noc] || 'N.A';
            stockinfo['massage'] +=
               element23 +
               'reduced to quantity:' +
               balances.Products[odd.noc][element23]['qty'] +
         ...
         notes +=
            '\n ' +
            todaysdate(6) +
            'Trading done for order of m/s' +
            odd.name;

         Object.keys(k2).forEach(function (element23) {
            balances.Procurement[odd.noc][element23]['qty'] =
               balances.Procurement[odd.noc][element23]['qty'] -
               k2[element23]['qty'];
            balances.Procurement[odd.noc][element23]['value'] =
               balances.Procurement[odd.noc][element23]['value'] -
               k2[element23]['value'];
            //set general,balances,odd
      ...
      var Ref231 = db
         .collection('Accounts/stockregister/' + odd.noc)
         .doc(todaysdate(2));
      ...
      if (
         odd.predelivery.invtyp == '1' ||
         odd.predelivery.invtyp == '3' ||
         odd.predelivery.invtyp == '5'
      )
         var Ref229 = db
            .collection('billofsupply')
            .doc(odd.postdelivery['docname']);
      else if (
         odd.predelivery.invtyp == '2' ||
         odd.predelivery.invtyp == '4' ||
         odd.predelivery.invtyp == '6'
      )
         var Ref229 = db
            .collection('invoices')
            .doc(odd.postdelivery['docname']);
   ...
   $('#neutralise').click(function () {
      notes +=
         '/n' +
         todaysdate(6) +
         '::Vehicles neutralised:' +
         Object.keys(general.vehiclesonduty).toString();
      ...
      swal(
         'Neutralized!',
         'Vehicles Neutralised-Now Empty!',
         'success'
      );
   });
   ...
   $('#notesaccess').click(function () {
      var tmp365 = new Date($('#notesdate option:selected').val())
         .getTime()
         .toString();
      console.log(tmp365);
      ...
      db.collection('notes')
         ...
         .then(function (doc) {
            ...
            if (typeof Storage !== 'undefined') {
               console.log('webstorage there');
               localStorage.setItem('note', doc.data().content);
            } else {
               console.log('Sorry! No Web Storage support..');
               document.cookie = doc.data().content;
            }
   ...
   $('#notesdelete').click(function () {
      ...
      db.collection('notes')
         ...
         .then(function () {
            ...
            swal('Sucessfull', 'Note Deleated!', 'success');
         });
   });
});
...
function getid3(idi) {
   ...
   var l1, l2, l3;
   if (ifcd[idi] != undefined) l1 = Object.keys(ifcd[idi]).length;
   else l1 = 0;
   if (ofcd[idi] != undefined) l2 = Object.keys(ofcd[idi]).length;
   else l2 = 0;
...
function getid32(idi) {
   ...
   var l1, l2, l3;
   if (ifud[idi] != undefined) l1 = Object.keys(ifud[idi]).length;
   else l1 = 0;
   if (ofud[idi] != undefined) l2 = Object.keys(ofud[idi]).length;
   else l2 = 0;
   ...
   db.collection('Orders')
      ...
      .then(function (doc) {
         ...
         Object.keys(k2).forEach(function (element22) {
            newtemp =
               balances.Products[doc.data().noc] ||
               balances.Procurement[doc.data().noc];
            console.log(newtemp);
            temp6 = `
               <br>     
    ...
    <input autocomplete="off" type="text" class="form-control" id="${
               element22 + 'qty'
               }" style="margin:4px;" value="${
               k2[element22]['qty']
               }" placeholder="Quantity" >
    <input autocomplete="off" type="text" class="form-control" id="${element22}value" style="margin:4px;" value="${
               k2[element22]['value']
               }" placeholder="Value">
        <h5>${newtemp[element22]['unit']}</h5>
...
function getid7(idi7) {
   notes +=
      '/n' +
      todaysdate(6) +
      '::stock value for ordere no ' +
      odd.postdelivery.invno +
      'changed from' +
      odd.stockinfo[idi7].toString() +
      'to';
   odd.stockinfo[idi7]['qty'] = parseFloat(
      $('#' + idi7 + 'qty').val()
   );
   odd.stockinfo[idi7]['value'] = parseFloat(
      $('#' + idi7 + 'value').val()
   );
   notes += 'to ' + odd.stockinfo[idi7].toString();
   details2 = {
      content: notes,
      date: todaysdate(5),
   };
...
function openNav() {
   ...
   //document.getElementById("main")p.style.marginLeft = "250px";
}
